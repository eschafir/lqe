...
def clean_query_text(text, domain=None):
    """Syntax-preserving query cleaner:
    Unescapes HTML entities, strips HTML tags, and removes standalone English stop-words
    without destroying LaTeX formulas, CLI flags, package names, or technical identifiers.
    """
    import re
    ...
    # 2. Token-aware stop-word filtering
    words = text.split()
    kept = []
...
def evaluate_split_hybrid(model, tokenizer, query_lut, dense_model, dense_processor, is_clip, device, 
    ...
    for row in tqdm(queries_ds, desc="Queries"):
        ...
        if filter_stopwords:
            q_text_cleaned = clean_query_text(q_text, domain=split_name)
        else:
            import re
            q_text_cleaned = re.sub(r'<[^>]+>', ' ', q_text).strip()
