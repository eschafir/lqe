...
def clean_query_text(text):
    # Remove HTML tags and punctuation, then split into words and filter stop words
    import re
...
# Limit queries to 20 for speed
queries_ds = list(queries_ds)[:20]

# Limit docs to a subset of 5000 containing the gold docs for the queries, to speed up encoding
gold_doc_ids = set()
for q in queries_ds:
...
with torch.no_grad():
    for start_idx in range(0, len(test_docs), 64):
        ...
        # Max-pool in chunks with logit threshold = 8.0
...
print(f"Average MRR@10 with Query Stopping: {mrr_total / count}")
