...
print("Encoding docs with Adapter...")
w_p_list_adapter = []
w_p_list_no_adapter = []
with torch.no_grad():
    for start_idx in range(0, len(test_docs), 64):
        ...
        z_p_adapter = torch.cat(z_p_parts, dim=1)
        w_p_list_adapter.append(z_p_adapter.cpu())
        
        # 2. Without Adapter (identity/pretrained)
        z_p_parts_no = []
        for start in range(0, vocab_size, 20000):
            weight_chunk = model.lm_head.weight[start : start + 20000]
            ...
            logits_chunk = logits_chunk * attention_mask + (1 - attention_mask) * -1e9
            z_p_chunk, _ = torch.max(logits_chunk, dim=1)
            z_p_parts_no.append(z_p_chunk)
        z_p_no_adapter = torch.cat(z_p_parts_no, dim=1)
        w_p_list_no_adapter.append(z_p_no_adapter.cpu())

z_p_adapter_all = torch.cat(w_p_list_adapter, dim=0)
z_p_no_adapter_all = torch.cat(w_p_list_no_adapter, dim=0)

def eval_config(z_p_all, use_query_lut):
    # Apply logit threshold = 8.0
    w_p_all = torch.log1p(torch.relu(z_p_all - 8.0))
    mrr_total = 0.0
    count = 0
    ...
    for q in queries_ds:
        ...
        with torch.no_grad():
            if use_query_lut:
                ...
                w_q_weights = torch.ones(len(query_token_ids), dtype=torch.float32) * 1.31
...
# Run 4 combinations
res1 = eval_config(z_p_no_adapter_all, use_query_lut=False) # Pretrained baseline
res2 = eval_config(z_p_no_adapter_all, use_query_lut=True)  # Pretrained + Query LUT
res3 = eval_config(z_p_adapter_all, use_query_lut=False)    # Adapter + Uniform Queries
res4 = eval_config(z_p_adapter_all, use_query_lut=True)     # Adapter + Query LUT

print("--- DIAGNOSTIC RESULTS (Threshold = 8.0) ---")
print(f"1. Pretrained Baseline:                 {res1:.4f}")
print(f"2. Pretrained Backbone + Query LUT:     {res2:.4f}")
print(f"3. Adapter Only + Uniform Queries:      {res3:.4f}")
print(f"4. Adapter + Query LUT (Checkpoint):    {res4:.4f}")
