...
docs_ds = load_dataset('mm-bright/MM-BRIGHT', "documents", split='biology')
queries_ds = load_dataset('mm-bright/MM-BRIGHT', "examples", split='biology')
...
# Calculate IDF for vocabulary tokens based on document texts
# To do this efficiently, we tokenize all documents and count token document frequencies
print("Calculating Document Frequencies (DF)...")
doc_freqs = collections.Counter()
for d in tqdm(test_docs, desc="Tokenizing docs for DF"):
...
with torch.no_grad():
    for start_idx in range(0, len(test_docs), 64):
        ...
        w_p = torch.log1p(torch.relu(z_p - 8.0))
        w_p_list.append(w_p.cpu())

w_p_all = torch.cat(w_p_list, dim=0)
...
for q in queries_ds:
    ...
    with torch.no_grad():
        ...
        # Apply IDF scaling to query tokens
        for i, token_id in enumerate(query_token_ids):
            token_idf = idf.get(token_id, math.log(1.0 + num_docs)) # default to max IDF if not seen
    ...
    mrr_total += mrr_val
    count += 1
    print(f"Query: {stopped_text[:60]}... | Gold Rank: {[ranked_doc_ids.index(g) if g in ranked_doc_ids else -1 for g in gold_ids]} | MRR@10: {mrr_val}")

print(f"Average MRR@10 with IDF Scaling: {mrr_total / count}")
