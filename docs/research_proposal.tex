...
\begin{itemize}
    \item \textbf{Caption RAG}: Vision-Language models pre-generate image captions. LQE runs regex matching over these captions to prevent visual vector search false positives (e.g., keeping the search locked onto \textit{``sinking boats''} rather than \textit{``sinking charts''}).
    \item \textbf{Discrete Visual Tokens}: Generative models represent images as grids of discrete visual codebook indices. The LQE harness can expand text queries into regex patterns of visual tokens (e.g., translating \textit{``blue boat''} to a sequence of color and edge tokens) and run \texttt{ripgrep} directly over raw tokenized image files.
\end{itemize}
