...
\midrule
Vanilla Grep & \textbf{70.0\%} & 717.1 & \textbf{86.67\%} & 1,239.1 \\
\textbf{LQE-Grep (v1)} & 60.0\% & 4,049.6 & \textbf{86.67\%} & 4,227.7 \\
\textbf{LQE-Grep v2 (Ours)} & 60.0\%* & \textit{TBD} & \textbf{86.67\%} & \textbf{2,293.4} \\
Vector Search & 0.0\% & \textbf{53.2} & 33.33\% & N/A \\
\bottomrule
\end{tabular}
\vskip 0.5em
\small * Note: LongMemEval LQE v2 execution is pending complete sweep.
\end{table}
...
\begin{itemize}
    \item \textbf{Caption RAG}: Vision-Language models pre-generate image captions. LQE runs regex matching over these captions to prevent visual vector search false positives (e.g. keeping the search locked onto *"sinking boats"* rather than *"sinking charts"*).
    \item \textbf{Discrete Visual Tokens}: Generative models represent images as grids of discrete visual codebook indices. The LQE harness can expand text queries into regex patterns of visual tokens (e.g., translating `"blue boat"` to a sequence of color and edge tokens) and run \texttt{ripgrep} directly over raw tokenized image files.
\end{itemize}
