...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{../results/evaluation_results.png}
\caption{LQE-Grep Evaluation Benchmark Suite: QA Accuracy and Token Footprint curves on the Synthetic, LongMemEval, and NFCorpus datasets.}
...
We conducted a pilot evaluation on the standard \textbf{ARO (Attribution, Relation, and Order) Visual Attribution dataset} \cite{yuksekgonul2022and} across 200 samples to test if CLIP-based embedding modifications could solve the attribute-binding bottleneck. 

The ARO dataset is specifically designed as a diagnostic tool for Visio-Linguistic models. For every image, it paired a \textbf{True Caption} (e.g., \textit{``the open door and the crouched man''}) with a \textbf{False Caption} (e.g., \textit{``the crouched door and the open man''}) generated by swapping the adjectives associated with the nouns. Because both captions share the exact same vocabulary words, this binary choice task prevents models from succeeding via simple keyword-matching shortcuts. To select the correct option, the model must actively verify the spatial and relational binding of attributes to their respective nouns. Consequently, models with no structural reasoning capabilities default to a baseline random-guess accuracy of exactly $50.00\%$.

Under this diagnostic setup, we evaluated five distinct M-LQE variants alongside Vanilla CLIP and VLM baselines:
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{../results/multimodal_evaluation_results.png}
\caption{Multimodal Compositionality \& Visual Attribution Benchmark Suite: comparative results of CLIP, M-LQE, and VLM on the ARO and Winoground datasets.}
\label{fig:multimodal_results}
\end{figure}
