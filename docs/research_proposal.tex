...
We conducted a pilot evaluation on the standard \textbf{ARO (Attribution, Relation, and Order) Visual Attribution dataset} \cite{yuksekgonul2022and} across 200 samples to test if CLIP-based embedding modifications could solve this bottleneck. We evaluated three methods:
\begin{enumerate}
    \item \textbf{Vanilla CLIP}: Cross-modal matching of the full caption against the image. (Accuracy: \textbf{55.50\%}).
    \item \textbf{Component Splitting}: Decomposing the query into segments (e.g., \textit{``open door''}, \textit{``crouched man''}), encoding them in isolation, and taking their average/product similarity. (Accuracy: \textbf{48.50\%} / \textbf{48.00\%}).
    \item \textbf{Grounded Object Cropping}: Physically cropping the image around the object's bounding box and matching the crop against object-adjective prompts. (Accuracy: \textbf{41.50\%}).
\end{enumerate}

These experiments revealed two vital limitations:
*   \textbf{Visual OOD Shift}: CLIP was pre-trained on full natural images, not on tiny, cropped patches. Zooming/cropping distorts scale and resolution, shifting the visual representation out-of-distribution (OOD) for the Vision Transformer (ViT) and degrading the embedding quality.
*   \textbf{Token Dilution and Noun Bias}: In isolated components, the noun (e.g., \textit{``door''}) dominates the embedding. When the image embedding is degraded, the relative difference of the adjectives (e.g., \textit{``open''} vs. \textit{``crouched''}) is washed out, and the classification is dominated by the text encoder's intrinsic vocabulary priors.
...
To validate this visual reasoning capability, we evaluated the \textbf{Llama-3.2-11B-Vision-Instruct} model on the ARO Visual Attribution benchmark (100 samples) via the NVIDIA NIM Cloud API. The VLM achieved an accuracy of \textbf{73.00\%} on the binary caption-selection task, representing a significant \textbf{+17.50\%} improvement over the Vanilla CLIP baseline (55.50\%). This confirms that the VLM's multi-modal cross-attention layers successfully resolve fine-grained attribute binding, validating its role as the high-precision indexing backbone for the M-LQE Caption-RAG system.
