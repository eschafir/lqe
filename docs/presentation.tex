    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \begin{tabular}{lrr}
                ...
                \midrule
                Vanilla Grep & \textbf{70.0\%} & 717.1 \\
                LQE-Grep (v1) & 60.0\% & 3,933.1 \\
                LQE-Grep v2 (Ours) & \textbf{60.0\%} & \textbf{2,897.7} \\
                Vector Search & 0.0\% & \textbf{53.2} \\
                \bottomrule
            \end{tabular}
            \vskip 0.8em
            \small
            * Evaluated on 10 random \texttt{single-session-user} queries from \texttt{longmemeval\_s\_cleaned.json}.
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{The Truncation Bottleneck}
                ...
                \begin{itemize}
                    ...
                    \item \textbf{LQE v2 Optimization}: Filters local conversational noise (e.g. \textit{``name''}, \textit{``shop''}) using turn frequency filtering, saving 26.3\% tokens.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \begin{tabular}{lrr}
                ...
                \midrule
                Vanilla Grep & \textbf{86.67\%} & 1,239.1 \\
                LQE-Grep (v1) & \textbf{86.67\%} & 4,227.7 \\
                LQE-Grep v2 (Ours) & \textbf{86.67\%} & \textbf{2,293.4} \\
                Vector Search & 33.33\% & N/A \\
                \bottomrule
            \end{tabular}
            \vskip 0.8em
            \small
            * Evaluated on 15 test queries comparing patient-written terms to medical documents.
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{The Precision Gap}
                \small
                \begin{itemize}
                    \item \textbf{Vector Search Fail}: Only 33.33\% Success@3 because embeddings blur the lines between related concepts (e.g. retrieving leukemia papers for *"Dragon's Blood"*).
                    \item \textbf{Lexical Precision}: Grep and LQE restrict matching to actual entity classes. LQE v2 global Document Frequency (DF) filtering cuts token inflation by 45.2\%.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
